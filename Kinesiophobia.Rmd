...
Kinphob_rho <- read.csv("2023-08-03_meta_analysis_kinesiophobia_rho.csv", header = TRUE, sep = ",")
```
...
```{r Forest plot kinesiophobia and PA, fig.height=11, fig.width=8}
...
## Meta-analysis: Post-hoc analysis (Spearman rho)

```{r secondary meta-analysis RE REML and Knapp-Hartung adjustment}
m.rho <- metacor(cor = rho, 
                 ...
                 method.tau = "REML",
                 hakn = TRUE,
                 title = "Kinesiophobia and physical activity behaviour based on Spearman rho")
summary(m.rho)
```

The post-hoc analysis based on Spearman’s rho values provided no evidence of an association between kinesiophobia and objective measures of physical activity. 

```{r Forest plot secondary analysis kinesiophobia and PA, fig.height=3, fig.width=8}
...
```{r Save Forest plot secondary analysis}
png(file = "Secondary analysis forestplot.png", width = 2000, height = 1100, res = 300)
...
```{r PA measure objectivity subgroup forest plot, fig.height=12, fig.width=8}
...
```{r Subgroup analysis by PA objectivity with Spearman rho}
Kinphob_rho$PA_objectivity <- as.factor(Kinphob_rho$PA_objectivity)

PAobj_subg_rho <- update.meta(m.rho, 
            subgroup = PA_objectivity, 
            tau.common = TRUE)
PAobj_subg_rho
```

```{r PA measure objectivity subgroup with Spearman rho forest plot, fig.height=12, fig.width=8}
forest.meta(PAobj_subg_rho, 
            layout = "RevMan5", 
            sortvar = -TE, 
            common = FALSE, 
            xlim = c(-1.0, 0.5), 
...
```{r Save PA objectivity whith Spearman rho forestplot}
png(file = "PA objectivity based on Spearman rho forestplot.png", width = 2800, height = 4500, res = 300)
...
```{r Forest plot for subgroup by kinesiophobia tool, fig.height=16, fig.width=8}
...
```{r PA outcome subgroup forest plot, fig.height=16, fig.width=8}
