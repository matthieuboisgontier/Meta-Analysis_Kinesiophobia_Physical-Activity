...
Egger’s regression test of funnel plot asymmetry (Egger et al., 1997) and a p-curve analysis (Simonsohn et al., 2014) were conducted to assess potential publication bias in our meta-analysis.

A secondary meta-analysis was conducted using the same approach, but based on Spearman’s rho values, to further test the relationship between kinesiophobia and device-based physical activity. We conducted exploratory subgroup analyses to examine the differences in correlations between studies including participants with different health conditions and using different types of physical activity measures (i.e., device-based versus self-reported), physical activity measurement instruments (i.e., type of questionnaires, type of devices), physical activity outcomes, and kinesiophobia measures. In addition, we used meta-regressions to examine if the average age of participants or the proportion of women in a study predicted the reported correlation between kinesiophobia and physical activity. 
...
Kinphob_r <- read.csv("meta_analysis_kinesiophobia_r_update5.csv", 
                      header = TRUE, sep = ",")
```
...
The sampling error variance on level 1 is small (10.3%). The value of I2 Level 2, the amount of heterogeneity variance within clusters (i.e. within studies), is small (78.2%). The largest share, however, falls to level 3. Between-cluster (here: between-study) heterogeneity makes up I2 Level 3 = 81.5% of the total variation in our data. Overall, this indicates that there is considerable between-study heterogeneity, and less than one tenth of the variance can be explained by differences within studies.
...
We see that the 3-level model, compared to the reduced one with 2 levels, shows a better fit. The Akaike (AIC) and Bayesian Information Criterion (BIC) are lower for the 3-level model, which indicates better performance and is consistent with the significant likelihood ratio test (LRT) comparing both models (χ2 = 12.67, p = 0.0004). Therefore, although the 3-level model introduces one additional parameter (i.e., it has 3 degrees of freedom instead of 2), this added complexity seems to be justified. Modeling of the nested data structure has improved our estimate of the pooled effect.
...
Our meta-analysis of 63 studies, 83 correlation estimates, and 12,278 participants revealed a statistically significant small-to-moderate negative correlation between kinesiophobia and physical activity (r = -0.19; 95% confidence interval [95% CI]: -0.26 to -0.13; p < 0.0001). However, we observed substantial-to-considerable between-study statistical heterogeneity (between-study Tau2 = 0.06, 95% CI: 0.02 to 0.09; I2 = 85.5%, 95% CI: 82.6 to 87.9%), and the prediction interval ranged from r = -0.61 to 0.30, indicating that a moderate positive correlation cannot be ruled out for future studies.
...
```{r Import rho data}
Kinphob_rho <- read.csv("meta_analysis_kinesiophobia_rho_update3.csv", 
                      header = TRUE, sep = ",")
```
...
Results of the secondary meta-analysis of 12 studies, 21 Spearman’s rho correlation estimates, and 2,084 participants was consistent with the main meta-analysis as it showed a statistically significant small-to-moderate negative correlation between kinesiophobia and physical activity (r = -0.20; 95% CI: -0.38 to -0.01; p = 0.049; Table 2; Figure 2). However, we observed substantial-to-considerable between-study statistical heterogeneity (Tau2 = 0.10, 95% CI: 0.04 to 0.28; I2 = 86.3%, 95% CI: 80.4 to 90.4%), and the prediction interval ranged from r = -0.71 to 0.45, indicating that a moderate positive correlation cannot be ruled out for future studies.
...
The value of I2 Level 2, the amount of heterogeneity variance within clusters (i.e. within studies), is very low, totaling roughly 1%. The largest share, however, falls to level 3. Between-cluster (here: between-study) heterogeneity makes up I2 Level 3 = 96.3% of the total variation in our data. Overall, this indicates that there is considerable between-study heterogeneity on the third level, and very little of the variance can be explained by differences within studies.
...
The test of subgroup differences between health status measures was possible between studies comprising people with chronic (k = 35) or acute pain (k = 2), arthritis (k = 11), a cardiovascular condition (k = 10), a neurological condition (k = 8), surgery (k = 5), older adults (k = 3), obstructive sleep apnea (k = 2), a pulmonary condition (k = 2), or fibromyalgia (k = 2) (Table 2; Figure 5). We found statistical differences between these studies (p = 0.0014). The relationship between kinesiophobia and physical activity was statistically significant only in studies that included participants with cardiovascular condition (r = -0.30; 95% CI: -0.47 to -0.11), arthritis (r = -0.25; 95% CI: -0.39 to -0.10), a neurological condition (r = -0.53; 95% CI: -0.69 to -0.32), a pulmonary condition (r = -0.68; 95% CI: -0.82 to -0.46), or older adults (r = -0.40; 95% CI: -0.60 to -0.14). We found no evidence of an association between kinesiophobia and physical activity in studies that included participants with chronic pain (r = -0.07; 95% CI: -0.16 to 0.01) or acute pain (r = -0.13; 95% CI: -0.45 to 0.23). Statistical heterogeneity was higher in the studies comprising people with a pulmonary condition (I2 = 98.1%), arthritis (I2 = 93.4%), or older adults (I2 = 91.2%) than the studies comprising people with a cardiovascular (I2 = 28.7%) or neurological condition (I2 = 55.9%).
...
PAobj_meta
```
The test of subgroup differences between self-reported (k = 54) and device-based (k = 29) measures of physical activity showed a statistically significant difference between these groups (p < 0.0001; Table 2). Both type of measures showed a negative correlation between kinesiophobia and physical activity, but self-reported measures showed a more negative correlation (r = -0.22; 95% CI: -0.29 to -0.14) than device-based measures (r = -0.13; 95% CI: -0.24 to -0.02; Supplemental Figure 6). However, we observed considerable between-study statistical heterogeneity (I2 = 85.5%). 
...
The test of subgroup differences according to the instruments used to measure physical activity was possible between studies using accelerometers (k = 27), IPAQ (k = 25), BHPAQ (k = 4), SGPALS (k = 4), ad-hoc questionnaires (k = 4), pedometers (k = 3), GPAQ (k = 2), MLTPAQ (k = 2), GLTEQ (k = 2), LTPAI (k = 2), SQUASH (k = 2), and RAPAQ (k = 2). We found no evidence of a moderating effect of physical activity instruments (p = 0.209). 
...
The test of subgroup differences between physical activity outcomes was possible between studies using MET-min per week typically from the IPAQ (k = 34), a score from a questionnaire (k = 16), the number of steps per day (k = 13), active time per day or week (k = 12), the number of counts per minute (k = 5), and the number of kilocalories per day (k = 2) (Table 2; Figure 9). We found no evidence of a moderating effect of physical activity outcome (p = 0.685). 
...
Kin_sub
```
The test of subgroup differences between instruments used to assess kinesiophobia was possible only for TSK-17 (k = 44), FABQ (k = 15), TSK-11 (k = 8), TSK-Heart (k = 7), TSK-11 (k = 8), and TSK-13 (k = 3) (Table 2; Figure 10). We found no evidence of a moderating effect of the instrument used to assess kinesiophobia (p = 0.452).
...
Age did not statistically influence the correlation values of the meta-analysis studies (k = 72; p = 0.349).
...
Similarly, the proportion of women (k = 72; p = 0.555) and the mean level of pain in the studies (k = 23; p = 0.481) did not influence correlation values. 
